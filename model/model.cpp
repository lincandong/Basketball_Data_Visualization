...
{
	vector<string> files2;
	cf_findFileFromDir2(".\\players", files2);
	char readBuffer[65536];
	for(auto& v : files2)
	{
		vector<player_avg*> temp;
...
void model::player_series(string name, string begin, string end, vector<player_avg*>& players)
{
	players.clear();
	if(m_player.find(name)!=m_player.end())
	{
...
void model::player_data(string name, string begin, string end, player_avg* p)
{
	int count = 0;
	p->name = name;
	if(m_player.find(name)!=m_player.end())
...
void model::team_data(string name, team_avg* p)
{
	p = m_team[name];
}

void model::order(int year, vector<player_avg*>& players, bool (*cmp)(player_avg*, player_avg*))
{
	players.clear();
	vector<player_avg*> temp = v_player[year];
	sort(temp.begin(), temp.end(), cmp);
...
void model::order(vector<team_avg*>& teams, bool (*cmp)(team_avg*, team_avg*))
{
	teams.clear();
	vector<team_avg*> temp = v_team;
	sort(temp.begin(), temp.end(), cmp);
